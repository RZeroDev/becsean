...
@section('content')
    <div class="container-fluid relative px-3">
        <div class="layout-specing">
            ...
            <div class="md:flex justify-between items-center">
                <div>
                    <h5 class="text-lg font-semibold">Modification de l'article: {{ $actuality->title }}</h5>

                    <ul class="tracking-[0.5px] inline-block mt-2">
                        {{ Breadcrumbs::render('actualities.edit', $actuality->slug) }}
                    </ul>
                </div>
                <div class="dropdown d-inline-block float-end mb-4">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
                        <li><a class="dropdown-item" href="{{ route('actualities.show', $actuality->slug) }}">Voir</a></li>
                        ...
                        <li>
                            <form action="{{ route('actualities.destroy', $actuality->slug) }}" method="POST"
                                ...
                                <a class="dropdown-item" href="#"
                                    onclick="deletePartner(event, {{ $actuality->id }})">Supprimer</a>
                            </form>
                        </li>
                    </ul>
                </div>
            ...
            <div class="grid md:grid-cols-12 grid-cols-1 mt-6 gap-6">
                <div class="lg:col-span-12 md:col-span-6">
                    ...
                    <div class="p-6 rounded-md shadow dark:shadow-gray-700 bg-white dark:bg-slate-900 mt-6">
                        ...
                        <form class="mt-6 myForm" action="{{ route('actualities.update', $actuality->slug) }}" method="POST"
                                    ...
                                    <div class="text-start">
                                        ...
                                        <div class="form-icon relative mt-2">
                                            ...
                                            <input name="titre" id="titre" type="text" required
                                                value="{{ old('titre', $actuality->title) }}"
    ...
    <select class="form-control @error('cate') is-invalid @enderror" name="cate" required="">
        @foreach($categorie as $cat)
            <option value="{{ $cat->id }}" {{ old('cate', $actuality->actualities_categorie_id) == $cat->id ? 'selected' : '' }}>
                {{ $cat->name }}
            </option>
        @endforeach
    </select>
                                    ...
                                    <div class="text-start">
                                        ...
                                        <div class="form-icon relative mt-2">
                                            <textarea name="contenu" id="contenu"
                                                ...
                                                placeholder="Message :">{{ old('contenu', $actuality->description) }}</textarea>
                                        </div>
                                        @error('contenu')
                                            <span class="text-danger">
                                                <strong>{{ $message }}</strong>
