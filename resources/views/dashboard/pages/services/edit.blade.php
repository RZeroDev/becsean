...
@section('title', 'Modification d\un service')
@section('content')
<div class="container-fluid relative px-3">
    <div class="layout-specing">
        <!-- Start Content -->
        <div class="md:flex justify-between items-center">
            <h5 class="text-lg font-semibold">Modification du service : {{ $service->title }}</h5>

            <ul class="tracking-[0.5px] inline-block sm:mt-0 mt-3">
                {{ Breadcrumbs::render('services.edit', $service->title) }}
            </ul>
        </div>
        ...
        <div class="grid lg:grid-cols-12 md:grid-cols-2 grid-cols-1 mt-6 gap-6">
            <div class="lg:col-span-12">
                <div class="p-6 rounded-md shadow dark:shadow-gray-700 bg-white dark:bg-slate-900">
                    ...
                    <form method="post" class="myForm" action="{{ route('services.update', $service) }}" enctype="multipart/form-data">
                    	@csrf
                    	@method('put')
                        <div class="grid lg:grid-cols-12 grid-cols-1 mt-6 gap-5">
                            <div class="lg:col-span-6">
                                ...
                                <input type="text" class="form-control @error('title') is-invalid @enderror" placeholder="Libellé du service" name="title" required value="{{ old('title', $service->title) }}">
                                @error('title')
                            ...
                            <div class="lg:col-span-12">
                                ...
                                <textarea class="form-input ps-11 w-full py-5 px-3 h-28 bg-transparent dark:bg-slate-900 dark:text-slate-200 rounded outline-none border border-gray-200 focus:border-indigo-600 dark:border-gray-800 dark:focus:border-indigo-600 focus:ring-0 " placeholder="Veuillez décrire le service" id="description" name="description">{{ old('description')?? $service->description }}</textarea>
                                @error('description')
                                    <span class="text-danger">
                                        <strong>{{ $message }}</strong>
                                    </span>
